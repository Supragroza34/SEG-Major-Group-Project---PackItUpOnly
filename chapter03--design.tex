...
\begin{figure}[htbp]
    ...
    \caption{System Diagram for the KCL Ticketing System}
...
The UML diagrams provide a visual representation of the system’s core entities and relationships, illustrating how components such as users, tickets, replies, and meeting requests interact in alignment with the Django ORM data model.
\begin{figure}[t]
    \centering
    \includegraphics[
            width=1.1\linewidth,
    ...
    \caption{UML Class Diagram for the KCL Ticketing System}
    \label{fig:system-architecture}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.6\textwidth}
        ...
        \includegraphics[
            width=1.0\linewidth,
            height=1.03\textheight,
            keepaspectratio
        ]{images/TicketCreationFlowXX.png}
        \caption{Ticket Creation Workflow}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.38\textwidth}
        ...
        \includegraphics[
            ...
            height=1.1\textheight,
            keepaspectratio
        ]{images/RequestMeetingFlowXX.png}
        \caption{Meeting Request Workflow}
    \end{subfigure}

    \caption{System workflows (part 1)}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.7\textwidth}
        ...
        \includegraphics[
            ...
            keepaspectratio
        ]{images/OfficeHoursFlowXX.png}
        \caption{Office Hours Workflow}
    \end{subfigure}

    \caption{System workflows (part 2)}
\end{figure}
...
This section discusses the principal design and implementation choices, how they appear in the delivered system, and why alternatives were not adopted. Where helpful, choices are related to established software engineering ideas: \emph{separation of concerns}, modular decomposition, risk and cost trade-offs, and maintainability under team development.

\subsection{Separation of frontend and backend (React and Django REST)}
The system was implemented as a React single-page application talking to a Django REST backend. This reflects a deliberate \emph{separation of concerns}: the UI (presentation, routing, client state) is isolated from server-side business rules, persistence, and authorisation. In practice, students and staff interact with page components under \texttt{src/pages/} and shared UI under \texttt{src/components/}. Tickets, meetings, and roles are enforced in Django views and serializers, creating a clear boundary that matches the three-tier structure described earlier.

\textbf{Alternatives considered.} A \emph{monolithic} Django application using server-rendered templates would have reduced the number of repositories and avoided CORS, aligning with a classical Model--View--Controller style within one process. It was rejected because rich, interactive dashboards (filtering, real-time feel, client-side validation) are more naturally expressed with a component-based SPA, and the course’s learning outcomes favoured a modern split-stack delivery. Lighter Python frameworks (e.g.\ Flask or FastAPI) were not chosen because Django’s built-in user model, admin patterns, and ORM reduced bespoke code for authentication and data modelling, improving delivery speed for a fixed-term group project.

\textbf{REST as the integration style.} Exposing a JSON REST API is a conventional \emph{client--server} contract: stable resources (\texttt{/api/tickets/}, \texttt{/api/staff/}, etc.) allow the frontend to evolve independently of URL structure and support future clients (e.g.\ a mobile app) without rewriting core logic. This is an application of the \emph{open--closed} idea at the API boundary.

\subsection{Generative AI: Google Gemini API versus self-hosted inference (e.g.\ Ollama)}
The AI helper is implemented by calling the \textbf{Google Gemini API} from the backend (see Section~\ref{sec:ai-helper-gemini}), not by running a large language model on the group’s own infrastructure.

\textbf{Why a managed API was chosen.} From a software engineering perspective, outsourcing inference to a managed API reduces \emph{operational risk}: the team does not provision GPUs, tune model serving, or patch inference runtimes. Integration is limited to HTTP requests, authentication, and prompt design, allowing for complexity that fits the ticketing domain without becoming an MLOps project. The end product reflects this as a thin server-side adapter (Django endpoint $\rightarrow$ Gemini) and a predictable failure mode (rate limits, timeouts) handled in application code.

\textbf{Why Ollama (or similar self-hosted stacks) was not used.} Early prototypes considered running a local or server-hosted model via \textbf{Ollama}, which would keep data on infrastructure the team controls and avoid per-token cloud pricing. This path was set aside for several reasons. Deploying Ollama for production-quality use implies either (i)~sufficient CPU/RAM on the deployment host for acceptable latency, or (ii)~GPU-backed cloud instances, which incur \emph{ongoing cost} and operational overhead (monitoring, restarts, model versioning). Coordinating consistent deployment across developers and a hosted environment would also have been harder than calling an API with a server-stored key. For a coursework timeline and team size, the managed API offered lower deployment friction and more predictable integration effort, at the expense of dependency on Google’s service and quota limits. Trade-offs are accepted and documented in the evaluation chapter.

\subsection{Authentication and data store choices}
\textbf{JWT.} Token-based authentication was chosen for stateless API access from the SPA. Alternatives include session cookies with CSRF protection for same-site deployments; JWT was preferred because it keeps authorisation explicit in API requests and matches common SPA tutorials and tooling, though it complicates token revocation without extra infrastructure (see below).

\textbf{SQLite versus PostgreSQL.} Local development uses SQLite to minimise setup; production uses Neon-hosted PostgreSQL. This follows the idea of \emph{environment parity} through the ORM: the same Django models run against both, reducing ``works on my machine'' defects while keeping developer onboarding lightweight.

\subsection{How decisions are reflected in the end product}
The architecture is not only conceptual: the repository layout mirrors it (React \texttt{src/} tree, Django \texttt{apps/}, REST routes in tables in Section~\ref{sec:api-design}), role-based access appears in both UI routes and permission classes, and the AI assistant is a concrete module (\texttt{AIChatbot}) rather than an ad hoc script. That alignment between design intent and implementation makes the system easier for future maintainers to navigate.

\subsection{Benefits of the chosen architecture}
